...
public class AppManager {
	...
	public void showAllPersons(){
		List<Person> persons = managePersonService.getPersons("all",null);
		personsGui(persons);
	}
	...
	public void getPersonByLastName(){
		...
		String name = in.next();
		List<Person> persons = managePersonService.getPersons("byLastName",name);
		personsGui(persons);
	}
	...
	public Person createPerson(){
		...
		return new Person(lastName, firstName, middleName, convertToGenderEnum(gender), date, isEmployed,
			Float.parseFloat(gwa));
	}

	public Role createRole(){
		Scanner in  = new Scanner(System.in);
		int count = 1;
		for (Role role : managePersonService.getRoles()){
			System.out.println(count + ". " + role.getRole());
			count++;
		}
		System.out.print("Select role 1-4 --> ");
		while(!in.hasNext("[1-4]$")){
			System.out.print("Please enter choice in the list --> ");
			in.nextLine();
		}
	...
	public void addPerson(){
		...
		managePersonService.execute("add",person);
	...
	public void updatePerson(){
		...
		managePersonService.execute("update",updatedPerson);
	...
	public void deletePersonById(){
		...
		Integer id = new Integer(in.nextInt());
		managePersonService.execute("deleteById",id);
	...
	public void personsGui(List<Person> persons) {
		System.out.println(String.format("%-5s%-30s%-10s%-20s%-13s%-10s%-6s%1s","+----","+-----------------------------","+---------","+-------------------","+------------","+---------","+-----","+"));
		System.out.println(String.format("%-5s%-30s%-10s%-20s%-13s%-10s%-6s%1s","|id","|full_name","|gender","|birthdate","|address_id","|employed","|gwa","|"));
		System.out.println(String.format("%-5s%-30s%-10s%-20s%-13s%-10s%-6s%1s","+----","+-----------------------------","+---------","+-------------------","+------------","+---------","+-----","+"));
		String date = null;
		for (Person p : persons){
			date = new SimpleDateFormat("MMMM-dd-yyyy").format(p.getBirthdate());
			System.out.println(String.format("%-5s%-30s%-10s%-20s%-13s%-10s%-6s%1s","|"+p.getId(),"|"+p.getFullName(),"|"+p.getGender(),"|"+date,"|"+p.getAddress().getPersonId(),"|"+p.isEmployed(),"|"+p.getGwa(),"|"));
		}
		System.out.println(String.format("%-5s%-30s%-10s%-20s%-13s%-10s%-6s%1s","+----","+-----------------------------","+---------","+-------------------","+------------","+---------","+-----","+"));
	}
	...
	public void addressGui(Address a) {
		...
		System.out.println(String.format("%-5s%-20s%-10s%-20s%-20s%-20s%-10s%1s","+----","+-------------------","+---------","+-------------------","+-------------------","+-------------------","+---------","+"));
		System.out.println(String.format("%-5s%-20s%-10s%-20s%-20s%-20s%-10s%1s","|"+a.getPersonId(),"|"+a.getStreet(),"|"+a.getHouseNo(),"|"+a.getBarangay(),"|"+a.getSubdivision(),"|"+a.getCity(),"|"+a.getZipCode(),"|"));
		System.out.println(String.format("%-5s%-20s%-10s%-20s%-20s%-20s%-10s%1s","+----","+-------------------","+---------","+-------------------","+-------------------","+-------------------","+---------","+"));
	}
